...
def _create_entry(img, question, answer, anno):
    if None != answer:
        answer.pop('image_id')
        answer.pop('question_id')
    if None != anno:
        anno.pop('image_id')
        anno.pop('question_id')
    entry = {
        'anno': anno,
...
def _load_dataset(dataroot, name, label2ans, ratio=1.0):
    ...
    """
    if name == 'train':
        question_path = os.path.join(dataroot, 'v2_OpenEnded_mscoco_train2014_questions.json')
    else:
    ...
    # anno
    anno_path = os.path.join(dataroot, 'cache11', '%s_target.pkl' % name)
    anno = cPickle.load(open(anno_path, 'rb'))
    anno = sorted(anno, key=lambda x: x['question_id'])[0:len(questions)]
    utils.assert_eq(len(questions), len(anno))
    ...
    entries = []
    for question, answer, anno in zip(questions_new, answers_new, anno):
        utils.assert_eq(question['question_id'], answer['question_id'])
        utils.assert_eq(question['question_id'], anno['question_id'])
        utils.assert_eq(question['image_id'], answer['image_id'])
        utils.assert_eq(question['image_id'], anno['image_id'])
        img_id = question['image_id']
        if not COUNTING_ONLY or is_howmany(question['question'], answer, label2ans):
            entries.append(_create_entry(img_id, question, answer, anno))
...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if None != answer:
            ...
            return features, spatials, question, target, question_id  # , bias
        else:
