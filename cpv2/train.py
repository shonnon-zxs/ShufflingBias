...
def cossi(a, b):
    apn = torch.cosine_similarity(a, b, dim=1)
    return apn
...
def logexp(ap, an):
    ...
    # torch.log1p()
    fm = torch.max(ap, an) + torch.log1p(torch.exp(-torch.abs(ap - an)))
    fz = torch.log(torch.exp(ap))
    return -(fz / fm)
...
def train(model, train_loader, eval_loader, opt):
    ...
    for epoch in range(opt.s_epoch, opt.num_epochs):
        ...
        for i, (v, b, q, a, _, bias) in enumerate(train_loader):
            v = v.cuda()
            q = q.cuda()
            a = a.cuda()
            bias = bias.cuda()
            ...
            if epoch < 0: #opt.pretrain_epoches:
                ...
                logits_pos, logits_neg1, logits_neg2, logits_neg3, _, _, _, _ = model(q, v, True)
                if opt.ml_loss:  # use multi-label loss
                    bce_loss_pos = instance_bce_with_logits(logits_pos, a, reduction='mean')
                else:  # use cross-entropy loss
                    bce_loss_pos = instance_bce(logits_pos, a)

                self_loss = compute_self_loss(logits_neg1, a) + compute_self_loss(logits_neg2, a) + compute_self_loss(
                    logits_neg3, a)

                loss = bce_loss_pos + self_loss
...
def evaluate(model, dataloader):
    ...
    num_data = 0
    entropy = 0
    for i, (v, b, q, a, q_id, _) in enumerate(dataloader):
        v = v.cuda()
        b = b.cuda()
        q = q.cuda()
        q_id = q_id.cuda()
        pred, att = model(q, v, None, None, False)
