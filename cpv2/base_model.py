...
class Model(nn.Module):
    ...
    def forward(self, q, gv_pos, a, bias, self_sup=True):
        ...
        logits_pos, att_gv_pos, joint_p = self.compute_predict(q_repr, q_emb, gv_pos)
        print(gv_pos)
        if self_sup:
            ...
            gv_neg = gv_pos[index]
            print(gv_neg)
            logits_neg, att_gv_neg, joint_n = self.compute_predict(q_repr, q_emb, gv_neg)
            loss = self.debias_loss_fn(joint_p, logits_pos, bias, a)
            return logits_pos, logits_neg, att_gv_pos, att_gv_neg, loss
        else:
            if bias is not None:
                loss = self.debias_loss_fn(joint_p, logits_pos, bias, a)
                return logits_pos, att_gv_pos, loss
            else:
    ...
    def compute_predict(self, q_repr, q_emb, v):
        ...
        logits = logits
        return logits, att_gv, joint_repr
